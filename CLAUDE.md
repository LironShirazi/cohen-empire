...
`:root` ו-`*` גלובליים, ובלי הסקופ ה-reset שלו (`margin/padding: 0`) דורס
את שאר המסכים. הפלטה המקומית שהייתה בו — `--cream`/`--green` משלו, וגם
הגדרות ל-`--ink`/`--line`/`--gold`/`--danger` שהסתירו את הגלובליים —
הוסרה: הצבעים, הרדיוסים והפונטים הם הטוקנים של `globals.css`.

⚠️ **פלטת הצמתים ב-SVG משוכפלת ב-`js/export.js`.** הייצוא בונה SVG עצמאי
עם CSS משלו, ולכן צבעי הצמתים, ההילה סביב הטקסט ורקע הקנבס כתובים שם
כ-hex קשיח. כל שינוי של `--ft-*` או של `--bg` חייב לגעת גם שם — אחרת
המסך מתעדכן וה-PNG/PDF נשארים ישנים.
