...
## 10. גלריית התמונות (שלב 3)

```
supabase/migrations/0012_phase3_gallery.sql  שנה, נתיב הקובץ, מדיניות כתיבה ו-bucket
web/app/gallery/page.tsx                     שער הכניסה + הרשאות הניהול
web/components/gallery/gallery-screen.tsx    ההעלאה והרשת לפי שנים
web/components/gallery/photo-lightbox.tsx    תצוגה מוגדלת, כותרת ומחיקה
web/lib/image.ts                             הקטנה ל-1600px לפני העלאה
```

**הגלריה מסודרת לפי `year` ולא לפי `race_id`.** ל-20 שנות המסורת שקדמו
לאפליקציה אין שורת `races` בכלל, ו-`race_id` הוא `on delete set null` —
כלומר מחיקת מירוץ הייתה מנתקת את התמונות שלו מהשנה שלהן. כשיש מירוץ,
הטריגר `sync_gallery_photo_year` הוא הקובע והקליינט לא יכול לטעון אחרת.

**ההעלאה פתוחה לכל בן משפחה מחובר, גם למירוץ בארכיון** — מעלים תמונות
אחרי המירוץ, וזו כל מהות הגלריה. הניהול (כותרת/מחיקה) הוא של מי שהעלה או
של מנהל, ו-`can_manage_gallery` מתרגם את זה. ⚠️ `is_race_admin` מחזיר
false למירוץ בארכיון, ולכן בפועל **מנהל-על** הוא שמנהל את הגלריה
ההיסטורית — המנהל התורן מנהל רק את המירוץ שהוא עדיין מנהל.

⚠️ **הרשאה ברמת העמודה, לא רק RLS.** ל-`authenticated` יש
`grant update (caption, year, race_id)` בלבד: RLS מחליט על אילו *שורות*
מותר לכתוב, לא על אילו עמודות, ובלי זה מי שהעלה תמונה יכול היה לשנות
בשורה שלו את `url` או את `uploaded_by`.

**סדר המחיקה: קודם השורה, אחר-כך הקובץ.** השורה היא מה שהמסך קורא —
אם נמחק קודם הקובץ ומחיקת השורה תיכשל, תישאר תמונה שבורה על המסך. הכיוון
