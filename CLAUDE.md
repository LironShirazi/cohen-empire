...
**מצב נוכחי: שלב 0 ושלב 1 מומשו.** אפליקציית Next.js חיה תחת `web/`
(ראו §5), הסכמה וה-RPC-ים ב-`supabase/migrations/`. `family-tree/` הוא
עדיין מודול עצמאי ב-vanilla JS שלא חובר ל-Supabase (שלב 3) — ראו §4.

השלב הבא לפי [docs/05-roadmap.md](docs/05-roadmap.md): **שלב 2** — צ'אט
קבוצתי, אזכורים והתראות, מפת מנהל חיה, חלוקה אוטומטית מאוזנת לקבוצות.
